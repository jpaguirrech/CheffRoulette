...
6. **Neon `DATABASE_URL`** — provision a Neon project under a stable owner account (not via the Replit↔Neon integration; tearing down the Repl deletes the DB). Postgres 16, region `us-east-1`. Save the **pooled** connection string as `DATABASE_URL` in Cloud Run env. Neon supports IP allowlisting, but Cloud Run egress is dynamic — use Neon's "allow all" or set up a Serverless VPC Connector + static egress IP if that's a concern. The same `DATABASE_URL` must be configured in the partner's extraction workflow at `flw.panteragpt.com`, since it writes recipes directly to Neon.
